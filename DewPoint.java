public class DewPoint implements NonLinearEquation {
    ...
    public DewPoint(DewPoint source){
    ...
    public DewPoint clone(){
        return new DewPoint(this);
    }

    public boolean equals (Object Comparator){
        if(!(this.getClass()==Comparator.getClass())) return false;
        for (int i = 0; i< this.flowStream.getFlowSpecies().size();i++){
            if(!(this.flowStream.getFlowSpecies().get(i)==((DewPoint) Comparator).flowStream.getFlowSpecies().get(i)));
            return false;
        }
        return true;
    }
    ...
    public double DewPointCalc() throws NoRootFoundException, ConvergenceNotMetException {
        double Tbubble = RootFinder.RiddersMethod(this,300,1000);
        if(Tbubble < 0 ) throw new NoRootFoundException("ERROR Bubble temperature is negative");
        if(Double.isNaN(Tbubble)) throw new NoRootFoundException("ERROR Bubble temperature is not a number");// replace with exception handling
        return Tbubble;
    }
    ...
    public double ReturnEquation(double X){
        ...
        for (i = 0; i < n; i++) {
            yi[i] = this.flowStream.getFlowSpecies().get(i).getOverallFraction();
